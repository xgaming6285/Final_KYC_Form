...
class VideoRecordingManager {
  ...
  async startRecording() {
    ...
    const isFaceVerificationPage =
      window.location.pathname.includes("face_verification");
    const cameraTypes = isFaceVerificationPage ? ["back"] : ["front", "back"];

    console.log(
      `🎥 Starting recording for cameras: ${cameraTypes.join(", ")} ${
        isFaceVerificationPage
          ? "(front camera reserved for face verification)"
          : ""
      }`
    );
    ...
    for (const cameraType of cameraTypes) {
      try {
        ...
        console.warn(
          `⚠️ Could not start ${cameraType} camera recording:`,
          error.message
        );
    ...
    if (activeRecordings > 0) {
      ...
      console.log(
        `✅ Video recording started with ${activeRecordings} camera(s)`
      );
  ...
  async startCameraRecording(cameraType) {
    try {
      ...
      const stream = await navigator.mediaDevices.getUserMedia(
        this.videoConstraints[cameraType]
      );
      ...
      mediaRecorder.ondataavailable = (event) => {
        if (event.data.size > 0) {
          ...
          console.log(
            `📊 Video chunk received for ${cameraType}: ${event.data.size} bytes (total chunks: ${this.videoChunks[cameraType].length})`
          );

          // Log total accumulated size periodically
          if (this.videoChunks[cameraType].length % 10 === 0) {
            const totalSize = this.videoChunks[cameraType].reduce(
              (sum, chunk) => sum + chunk.size,
              0
            );
            console.log(
              `📈 ${cameraType} camera accumulated: ${
                this.videoChunks[cameraType].length
              } chunks, ${(totalSize / 1024 / 1024).toFixed(2)} MB total`
            );
          }
        } else {
      ...
      mediaRecorder.onstop = () => {
        ...
        const chunkCount = this.videoChunks[cameraType]
          ? this.videoChunks[cameraType].length
          : 0;
        ...
        if (chunkCount > 0) {
          const recordingDuration = this.recordingStartTime
            ...
            : 0;
          console.log(
            `📤 Uploading final ${cameraType} camera video (${recordingDuration.toFixed(
              1
            )}s, ${chunkCount} chunks)`
          );
      ...
      mediaRecorder.onerror = (event) => {
        console.error(
          `❌ MediaRecorder error for ${cameraType} camera:`,
          event.error
        );
      };
      ...
      console.warn(
        `⚠️ Could not access ${cameraType} camera for recording:`,
        error.message
      );
      ...
      if (error.name === "NotAllowedError") {
        ...
        console.warn(
          `Camera already in use or hardware error for ${cameraType} camera`
        );
      }
  ...
  stopRecording() {
    ...
    setTimeout(() => {
      ...
      console.log(
        `✅ Recording stopped after ${recordingDuration.toFixed(1)} seconds`
      );

      // Final video upload is now handled by "Complete KYC" button
      console.log(
        "ℹ️ Video recording stopped - final upload handled by Complete KYC button"
      );
  ...
  async uploadVideo(cameraType) {
    if (
      !this.videoChunks[cameraType] ||
      this.videoChunks[cameraType].length === 0
    ) {
    ...
    try {
      ...
      const mimeType =
        this.videoExtension === "mp4" ? "video/mp4" : "video/webm";
      const videoBlob = new Blob(this.videoChunks[cameraType], {
        type: mimeType,
      });

      // Check if blob has meaningful size (more than just headers)
      if (videoBlob.size < 512) {
        // Less than 512 bytes is likely empty
        console.warn(
          `⚠️ Video blob too small (${videoBlob.size} bytes) for ${cameraType} camera - skipping upload`
        );
        return;
      }
      ...
      const recordingDuration = this.recordingStartTime
        ...
        : 0;
      console.log(
        `📤 Uploading ${cameraType} camera video (${(
          videoBlob.size /
          1024 /
          1024
        ).toFixed(2)} MB, ${recordingDuration.toFixed(1)}s)...`
      );
      ...
      formData.append(
        "video",
        videoBlob,
        `session_${cameraType}_camera.${this.videoExtension}`
      );
      ...
      if (result.success) {
        console.log(
          `✅ ${cameraType} camera video uploaded successfully: ${result.s3_key}`
        );
      } else {
        console.error(
          `❌ Failed to upload ${cameraType} camera video:`,
          result.error
        );
      }
    } catch (error) {
  ...
  isCameraRecording(cameraType) {
    return (
      this.mediaRecorders[cameraType] &&
      this.mediaRecorders[cameraType].state === "recording"
    );
  }
    ...
    try {
      ...
      Object.keys(this.videoChunks).forEach((cameraType) => {
        if (this.videoChunks[cameraType]) {
          ...
          state.totalSize[cameraType] = this.videoChunks[cameraType].reduce(
      ...
      console.log(
        `💾 Video state saved: ${
          Object.keys(state.chunkCounts).length
        } cameras, total chunks: ${Object.values(state.chunkCounts).reduce(
          (a, b) => a + b,
          0
        )}`
      );
      ...
      if (
        ...
        try {
          ...
          if (videoBlob.size > 1024) {
            // Only backup if meaningful size
            ...
            fetch("/save_video_backup", {
              ...
              .then((response) => {
                if (response.ok) {
                  console.log(
                    `💾 Local backup saved for ${cameraType} camera (${(
                      videoBlob.size /
                      1024 /
                      1024
                    ).toFixed(2)} MB) - S3 upload on session end`
                  );
                }
              })
              .catch((error) => {
  ...
  async uploadFinalVideoToS3() {
    ...
    Object.keys(this.videoChunks).forEach((cameraType) => {
      if (
        ...
        console.log(
          `📹 ${cameraType} camera: ${chunkCount} chunks, ${(
            totalSize /
            1024 /
            1024
          ).toFixed(2)} MB`
    ...
    console.log(
      `🎯 Selected ${bestCameraType} camera for final upload (${totalChunks} total chunks, ${(
        maxSize /
        1024 /
        1024
      ).toFixed(2)} MB)`
    ...
    try {
      ...
      if (this.isRecording) {
        ...
        maxSize = this.videoChunks[bestCameraType]
          ? this.videoChunks[bestCameraType].reduce(
              (sum, chunk) => sum + chunk.size,
              0
            )
          : 0;
        console.log(
          `📊 Updated size after final data collection: ${(
            maxSize /
            1024 /
            1024
          ).toFixed(2)} MB`
      ...
      const mimeType =
        this.videoExtension === "mp4" ? "video/mp4" : "video/webm";
      const finalVideoBlob = new Blob(this.videoChunks[bestCameraType], {
        type: mimeType,
      });

      console.log(
        `📤 Creating final video blob: ${(
          finalVideoBlob.size /
          1024 /
          1024
      ...
      if (finalVideoBlob.size < 1024) {
        // Must be at least 1KB to be meaningful
        console.log(
          `❌ Final video blob too small (${finalVideoBlob.size} bytes) - likely empty recording`
        );
        return {
          success: false,
          error: "Final video recording is too small or empty",
        };
      }

      // Create form data for final upload
      const formData = new FormData();
      const filename = `final_session_${this.sessionId}.${
      ...
      if (response.ok && result.success) {
        ...
        console.error(
          `❌ Failed to upload final video to S3:`,
  ...
  /**
   * Force immediate cleanup of all camera resources (for face verification)
   */
  async forceCleanupForFaceVerification() {
    console.log("🚨 Force cleanup for face verification initiated...");

    // Immediately mark as not recording
    this.isRecording = false;

    // Force stop all MediaRecorder instances immediately
    const recorderStopPromises = [];
    if (this.mediaRecorders) {
      for (const cameraType of Object.keys(this.mediaRecorders)) {
        const recorder = this.mediaRecorders[cameraType];
        if (recorder && recorder.state !== "inactive") {
          console.log(`🛑 Force stopping ${cameraType} MediaRecorder...`);
          ...
          const stopPromise = new Promise((resolve) => {
            if (recorder.state === "inactive") {
              ...
              // Shorter timeout for force cleanup
              setTimeout(resolve, 500);
            }
          });

          recorderStopPromises.push(stopPromise);
          recorder.stop();
        }
      }
    }
    ...
    await Promise.all(recorderStopPromises);
    console.log("✅ All MediaRecorders force stopped");

    // Force stop all camera streams with verification
    if (this.streams) {
      for (const cameraType of Object.keys(this.streams)) {
        const stream = this.streams[cameraType];
        if (stream) {
          console.log(`🎥 Force releasing ${cameraType} camera stream...`);
          ...
          for (const track of tracks) {
            if (track.readyState !== "ended") {
              track.stop();
              console.log(
                `✋ Force stopped ${track.kind} track for ${cameraType}`
              );

              // Verify track actually ends
              let attempts = 0;
              while (track.readyState !== "ended" && attempts < 20) {
              ...
              if (track.readyState === "ended") {
                console.log(
                  `✅ ${track.kind} track for ${cameraType} confirmed ended`
                );
              } else {
                console.warn(
                  `⚠️ ${track.kind} track for ${cameraType} may not have ended cleanly`
                );
              }
            }
          }
    ...
    console.log("🧹 Force cleanup completed - all camera resources released");
